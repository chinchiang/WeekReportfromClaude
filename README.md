# 資安合規雙週報 Security & Compliance Biweekly

每兩週自動彙整資訊安全 / 網路安全重大事件、新聞與法遵動態的動態網站。
...
reports/index.json    # 期數索引（新的排最前面）
reports/<id>.json     # 每期一份報告資料，id 格式：YYYY-Www（如 2026-W34）
```

網站於載入時讀取 `reports/index.json` 取得期數清單，再依選擇載入對應的
`reports/<id>.json` 渲染內容，支援深/淺色主題、中英雙語與期數存檔切換。
...
**續報標示**：持續追蹤且本期無實質新進展的項目，標題加「【續報】/ [Follow-up]」前綴，讓讀者區分新聞與追蹤提醒。
...
**發布頻率：每兩週一次（隔週一 08:00）。**
排程每週一喚醒，先以 ISO 週次判斷：**奇數週**產出報告、**偶數週**直接跳過
（cron 無法原生表達「隔週」，且同時限制日期與星期在不同 cron 實作下語義不一致，
因此改以週次奇偶做閘門，可保證固定 14 天間隔）。

產報週執行以下步驟：

1. 以網路搜尋彙整**過去兩週**上述各主題的重大事件、新聞、法令動態
2. 依現有 schema 新增 `reports/YYYY-Www.json`（Www 取涵蓋範圍中「較晚」的 ISO 週次，
   例如涵蓋 W33–W34 命名 `2026-W34`；`period` 寫完整兩週範圍）：
   - **雙語內容**：`title`、`summary`、`highlights[]`、`sections[].name`、item 的
     `title`/`content`/`action` 及含中文的 `date`，一律使用 `{ "zh": "...", "en": "..." }`
     物件（純日期或英文專名可用字串，網站兩種語言都會直接顯示）
   - `highlights`：本期 3–5 條焦點摘要
   ...
   - 某主題當期無重大更新時，`items` 留空陣列即可
3. 在 `reports/index.json` **最前面**插入新一期的索引項目（title 如「2026 第 33–34 週」）
4. Commit 並 push
